...
Theorem A is proved in \S\ref{sec:universal}. It is stated for multigraphs, which
is formally stronger than the question as posed and in particular covers the
example \eqref{eq:lps}; loops, bridges, and pairs of parallel marked edges are
included rather than excluded, as \S\ref{sec:degenerate} records.
...
Proposition D1 is an exact finite identity, and Corollary D2 turns the whole
complete-graph problem into a two-sided estimate of $\E[L_n]$; the two halves
turn out to be of very different quality (\S\ref{sec:identity}). The identity also explains why
the same constant $10-4\zeta(3)$ appears in the PWIT calculation of \cite{TZ}:
the finite complete-graph second moment is tied exactly to the classical
minimum-spanning-tree weight. Corollary D3 and its rate use the asymptotic
results of \cite{CFIJS,LiZhang}; Theorems B1 and B2 remain finite and effective
at every $n$.
...
The two models are different, and
Remark~\ref{rem:attenuation} quantifies the difference: the natural way to
realise a $\max(U_1,\dots,U_t)$ law inside an i.i.d.\ model, namely subdividing
an edge into a path, does not preserve the event ``one fixed edge lies in the
tree'', and the resulting attenuation destroys exactly the amplification that
Theorem C produces.
...
--- rewrites $H$ as $n-1/n+J$, where $J$ sums $1/|C|$ over the components $C$
created by mergers; since a created component has at most $n$ elements,
$J\ge(n-1)/n$ pathwise, and that one line is Theorem B1. The other direction
...
A natural attempt to enlarge the ratio is to iterate the construction of
\cite{LPS}, enlarging the two parallel bundles. It does not work, and the
failure is quantitative. Let $R_{r,s}$ denote the ratio for one marked copy in
...
describes an independent exact check of the formulas and certificate.
...
probabilities and hence the ratio. Appendix~\ref{sec:verification} records a
second exact enumeration by relative-order patterns.
\end{proof}
...
exact census of all such graphs with $m\le7$ finds no violation; the census is
recorded in Appendix~\ref{sec:verification}.
...
the only place where the complete graph enters. Appendix~\ref{sec:verification}
checks the transition model against a direct evaluation of $\PMST$.
...
Every parent size in a merger history is at most $n$, and there are $n-1$
mergers, so pathwise
\begin{equation}
\label{eq:Jlower}
J=\sum_{\text{mergers}}\frac1{a+b}\ \ge\ \frac{n-1}n ,
\end{equation}
with strict inequality for $n\ge3$ because the first merger has parent size
$2<n$. By Lemma~\ref{lem:HJ}, $H\ge n-1/n+(n-1)/n=n+(n-2)/n$ pathwise, and hence
by Lemma~\ref{lem:drift}
\begin{equation}
\label{eq:Phiupper}
\E[\Phi]\ \le\ 8(n-1)-4n-\frac{4(n-2)}n=\frac{4(n-1)(n-2)}n ,
\end{equation}
strictly for $n\ge3$. By vertex exchangeability, if $D$ is the degree of a fixed
vertex then $\E[D(D-1)]=\E[\Phi]/n$. Finally, $K_n$ has exactly $n(n-1)(n-2)$
...
$\E[\Phi]$, so
\[
p_1=\frac{\E[\Phi]}{n(n-1)(n-2)}<\frac1{n(n-1)(n-2)}\cdot\frac{4(n-1)(n-2)}n
=\frac4{n^2}=p_0^2 . \qedhere
\]
\end{proof}

The adjacent case thus rests on a single pathwise observation: a component
created by a merger has at most $n$ elements.
...
\begin{remark}
In the language of \cite[Corollary 2.4]{TZ}, which the counting in this section
and Corollary D2 reprove: with $D$ the degree of a fixed vertex,
$p_1\le p_0^2\iff\E[D^2]\le6-14/n+8/n^2$ and
$p_2\le p_0^2\iff\E[D^2]\ge5-11/n+6/n^2$. So the two halves of Corollary B3
squeeze $\E[D^2]$ into a window of width $1-3/n+2/n^2$ around $5$--$6$; the
limiting window is $(5,6)$ and the true limit is $10-4\zeta(3)=5.1917\ldots$ by
Corollary D3.
\end{remark}
...
The lower estimate follows for every $n\ge2$ from \eqref{eq:Jlower}, and the
upper estimate from Lemma~\ref{lem:budget}; their equality statements follow
from the strictness statements there.
\end{proof}

Thus Theorem B1 is the statement $\E[L_n]\ge(n-1)(n+2)/n^2$ and Theorem B2 is the
statement $\E[L_n]\le(n-1)(5n+6)/(4n^2)$, and \S\ref{sec:kn} proves both without
any input from the minimum-spanning-tree-weight literature. Numerically the lower
bound tends to $1$ and is far from tight, while the upper bound tends to $5/4$
and is tight to about $1\%$: see Table~\ref{tab:exact}. Thus the adjacent case,
which requires the lower bound on $\E[L_n]$, is resolved here by the looser of
the two estimates. Both pair ratios increase in the computed range. The maximum
...
\toprule
$n$ & $p_1/p_0^2$ & $p_2/p_0^2$ & $\E[L_n]$ & lower bd.\ $\frac{(n-1)(n+2)}{n^2}$
...
\bottomrule
\end{tabular}
\caption{Exact pair correlations for $\MST(K_n)$ and the two-sided estimate of
$\E[L_n]$ from Corollary D2. Every entry comes from an exact rational; the
$\E[L_n]$ column is rounded to four decimals. The full exact table for
$3\le n\le30$ accompanies the paper.}
\label{tab:exact}
\end{table}
...
enters in three places: it supplies the exact census of Table~\ref{tab:exact},
proves the finite enumerations in Propositions~\ref{prop:simple-counterexample}
and \ref{prop:hub-witnesses}, and checks the algebra of
Proposition~\ref{prop:bundle} and the transition model of
Lemma~\ref{lem:coalescent} against direct evaluations of $\PMST$. Every
quantity is an exact rational; decimals are rounded only when printed. The full
table for $3\le n\le30$ accompanies the paper as
\texttt{data/kn-exact-table.csv}. The code uses only the Python standard
library, and the repository records the commands and provenance.

\subsection{What each computation evaluates}

\begin{enumerate}
\item \emph{Brute force over orderings.} For a small multigraph, enumerate all
$m!$ orderings of the edges, run Kruskal on each, and count. This evaluates
$\PMST$ directly from its definition. It is used on $K_4$ (720 orders), the
five-vertex graph of Proposition~\ref{prop:simple-counterexample} (40320
orders), the two-hub graph of Proposition~\ref{prop:hub-witnesses} ($10!$
orders), and small multigraphs.
\item \emph{Exact summation over order patterns.} Condition on the set of edges
lighter than $e$, or, for $p_{ef}$, on the three blocks into which the two marked
weights split the remaining $m-2$ edges, and sum the exact rational probability
of each pattern. This too evaluates $\PMST$ directly, by a different
enumeration, and reaches $K_5$, both simple-graph propositions, and the example
\eqref{eq:lps}.
\item \emph{The accepted-merger coalescent.} The dynamic program over integer
partitions given by \eqref{eq:mcrate}, returning $\E[J]$, $\E[H]$, $\E[\Phi]$,
$\E[D^2]$, $p_1$, $p_2$ and $\E[L_n]$ exactly. This is the machinery of
\S\ref{sec:kn}, so it is not independent evidence for \S\ref{sec:kn}; it is what
routes 1 and 2 are used to test.
\item \emph{$\E[L_n]$ from the area identity.} Independently of the coalescent,
$\E[L_n]=\int_0^\infty(\E[\kappa(t)]-1)\,dt$ where $\kappa(t)$ counts components
of $G(n,1-e^{-t})$; substituting $u=e^{-t}$ turns the integrand into an integer
polynomial in $u$ built from the exact connectivity polynomial of $G(s,p)$, so
the integral is a finite rational sum. This route knows nothing about mergers,
degrees, or the coalescent.
\end{enumerate}

Lemma~\ref{lem:coalescent} is checked from outside by these routes: routes 1 and
2 agree with each other on every marginal and every pair probability of $K_4$,
and both agree with route 3 on $p_1$, $p_2$, on $\E[\Phi]=8(n-1)-4\E[H]$ and on
Proposition D1 at $n=4$ and $n=5$; route 4 agrees with route 3 on $\E[L_n]$; and
route 3 agrees with the published table of \cite{Gamarnik} through $n=30$ in
the full suite (the committed transcription extends through $n=45$). The
self-consistency checks $p_0=2/n$, $\sum_e p_e=n-1$ and $\E[H]=n-1/n+\E[J]$ are
also verified, the last recomputed from the dynamic program rather than assumed.

\subsection{What is verified, and in what range}

The lemmas of \S\ref{sec:kn} are verified exhaustively in finite ranges:
Lemma~\ref{lem:HJ} and the pathwise bound \eqref{eq:Jlower} on every merger
history shape up to $n=12$; Lemma~\ref{lem:partition}, together with its equality
characterisation, on every integer partition of every $n\le32$; and the closing
algebra of \S\ref{sec:kn} and \S\ref{sec:identity} as rational identities for
$n<300$. The chain of inequalities in the proof of Theorem A is verified,
instance by instance and exactly, on an explicitly generated exhaustive universe
of connected labelled multigraphs --- including loops, parallel classes and
disconnected deleted environments --- for every marked pair, together with the
frozen-environment forms of Lemmas~\ref{lem:harris} and \ref{lem:sandwich}
obtained by exact integration over the two marked weights. The bundle family of
Proposition~\ref{prop:bundle} is evaluated for all $r+s\le120$ in the widest
documented run, and its two positive-coefficient certificates are re-expanded
coefficient by coefficient. The positive-coefficient certificate for
$R(t+1)>R(t)$ is likewise re-expanded; an additional finite scan reaches
$t=300$ in the widest run. The two- and three-hub probabilities are recomputed
by relative-order enumeration, and the two-hub graph also by all $10!$ orders.
The minimality statement after Proposition~\ref{prop:simple-counterexample}
uses the bridge reduction printed there and a census of 1528 labelled
bridgeless graphs with at most seven edges, collapsing to 17 isomorphism
classes and 272 representative marked pairs. Finally, the formulas of Theorem C
and Remark~\ref{rem:attenuation} are checked directly on the corresponding
i.i.d.\ subdivided graphs for $t=1,2,3$.
...
These finite checks do not replace the proofs above. They test the modelling
and algebra and establish the explicitly finite assertions: the exact table,
the enumerated counterexamples, and the edge-count minimality census.
